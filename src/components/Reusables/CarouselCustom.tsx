  ...
  return (
    <div className={`${className} ${pause === true ? 'bg-white bg-opacity-[0.1] rounded-lg' : ''}  flex flex-col items-center space-y-4`}>
      <div className="relative h-96 w-full border-2 rounded-lg border-secondary dark:border-secondarydark flex items-center p-1">
        <button onClick={prevMedia} className="absolute left-1 border-none fill-white hover:fill-gray-400 transition-colors duration-150 focus:outline-none p-2">
        ...
        <button onClick={nextMedia} className="absolute right-1 border-none fill-white  hover:fill-gray-400 transition-colors duration-150 focus:outline-none p-2">
        ...
        {imgList.map((media, index) => (
          <div
            className={`${index === activeIndex ? 'block' : 'hidden'} w-full h-full flex items-center justify-center mx-14`}
            key={index}
          >
